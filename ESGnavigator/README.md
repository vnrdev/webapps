The ESG Navigator is an Enterprise Social Governance questionnaire tool written in ReactJS.

### Libraries
- ReactJS
- TypeScript
- TanStackQuery
- Next.js
- TailWind
- IBM Carbon Design

Question
![question](Screenshots/large-image.png)

Question type: free text
![question2](Screenshots/question.png)

Question type: checklist (tablet)
![question_check](Screenshots/question_check_tablet.png)

Question type: date
![question_date](Screenshots/question_date.png)

Question type: free text (mobile)
![question_freetext](Screenshots/question_freetext.png)

Question type: question list
![question_list](Screenshots/question_list.png)

Question type: question list (tablet)
![question_list_2](Screenshots/question_list_2.png)

Question type: question list (mobile)
![question_list_mobile](Screenshots/question_list_mobile.png)

Question type: number
![question_number](Screenshots/question_number.png)

Question type: rating
![question_rating](Screenshots/question_rating.png)

Question type: rating (mobile)
![question_rating_mobile](Screenshots/question_rating_mobile.png)

Question type: rating (tablet)
![question_rating_tablet](Screenshots/question_rating_tablet.png)

Question type: y/n
![question_yn](Screenshots/question_yn.png)


